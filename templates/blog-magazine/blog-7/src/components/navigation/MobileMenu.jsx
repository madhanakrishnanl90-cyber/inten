...
export function MobileMenu({ isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 lg:hidden flex">
      ...
      {/* Drawer */}
      <div className="relative w-4/5 max-w-sm bg-[#FAF9F5] h-full shadow-2xl z-10 flex flex-col justify-between overflow-y-auto border-r border-[#141413]">
        <div>
          {/* Header */}
          <div className="p-4 border-b border-[#E8E5DC] flex items-center justify-between">
          ...
          <div className="p-4 border-b border-[#E8E5DC] space-y-2">
            <button
              ...
              className="w-full flex items-center gap-2 px-3 py-2 bg-white border border-[#D1CDC4] text-xs text-[#73736C] font-medium"
            >
            ...
            <div className="space-y-1">
              ...
              {categories.map((cat) => (
                <Link
                  ...
                  onClick={onClose}
                  className="block py-1.5 text-sm font-medium text-[#4A4A45] hover:text-[#141413]"
                >
                  {cat.name}
                </Link>
              ))}
        ...
        <div className="p-4 border-t border-[#E8E5DC] bg-[#F4F1EA]">
          <button
            ...
            className="w-full py-2.5 bg-[#141413] text-[#FAF9F5] text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 hover:bg-[#D43825] transition-colors"
          >
            <Sparkles className="w-3.5 h-3.5" />
            <span>Join The Dispatch</span>
          </button>
          <p className="text-[0.65rem] text-[#73736C] text-center mt-2">
