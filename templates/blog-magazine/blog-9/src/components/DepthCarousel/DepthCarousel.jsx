...
const DEFAULT_ITEMS = [
  { image: 'https://images.unsplash.com/photo-1518709268805-4e9042af9f23?auto=format&fit=crop&w=800&q=80', alt: 'Slide 1', title: 'The Unknown Earth', tag: 'September 2026', link: '/magazine/september-2026' }
];
      ...
      <div className="depth-carousel__stage" ref={stageRef}>
        {data.map((item, i) => (
          ...
          >
            ...
            {showMetadata && (item.title || item.tag) && (
              <div className="depth-carousel__content-overlay">
                {item.tag && <div className="depth-carousel__tag">{item.tag}</div>}
                {item.title && <h3 className="depth-carousel__title">{item.title}</h3>}
                {item.dek && <p className="depth-carousel__dek">{item.dek}</p>}
                {item.link && (
                  <button
                    type="button"
