import { aurelisImages } from './aurelisImages';

export const aurelisData = {
  hero: {
    eyebrow: 'AURELIS • ENTERPRISE SOLUTIONS',
    headline: 'Strategy That Creates Lasting Impact.',
    paragraph: 'We partner with enterprise organizations to structure long-term growth, optimize boardroom governance, and deliver sustainable value to global shareholders.',
    ctaPrimary: 'EXPLORE SERVICES',
    ctaSecondary: 'CONTACT US',
    image: aurelisImages.hero,
    floatingCard: {
      number: '98%',
      label: 'Strategy Adoption Rate'
    }
  },
  about: {
    label: 'ABOUT AURELIS',
    title: 'Creating Trust and Value Through Decades of Leadership.',
    paragraph1: 'Aurelis is a premier global advisory firm. We help enterprise boards and executive teams solve complex corporate challenges, execute mergers, and manage institutional risk.',
    paragraph2: 'With offices in key financial hubs, we combine macro market analytics with deep sector experience to formulate actionable growth roadmaps.',
    image: aurelisImages.about
  },
  services: [
    {
      number: '01',
      title: 'Corporate Strategy',
      desc: 'Custom corporate frameworks designed to maximize market share, streamline operations, and secure long-term revenue streams.'
    },
    {
      number: '02',
      title: 'Risk Advisory & Governance',
      desc: 'Comprehensive compliance audit services, financial risk modelling, and sound governance designs for institutional boards.'
    },
    {
      number: '03',
      title: 'Mergers & Acquisitions',
      desc: 'End-to-end deal structure, financial due diligence, synergy modeling, and post-merger integration planning.'
    },
    {
      number: '04',
      title: 'Capital Restructuring',
      desc: 'Optimizing balance sheets, refinancing structures, and advising on equity and debt issuance frameworks.'
    }
  ],
  industries: [
    { name: 'Financial Services', desc: 'Commercial banks, asset management, insurance groups.' },
    { name: 'Energy & Power', desc: 'Transition strategies, infrastructure restructuring, grids.' },
    { name: 'Global Logistics', desc: 'Supply chains, warehousing pipelines, shipping terminals.' },
    { name: 'Life Sciences', desc: 'Pharmaceutical compliance, board oversight, expansion.' }
  ],
  stats: [
    { value: 25, label: 'Years of Strategy' },
    { value: 180, label: 'Billion Transaction Value' },
    { value: 45, label: 'Average Growth Rate' },
    { value: 94, label: 'Client Retention' }
  ],
  leadership: [
    {
      name: 'Sarah Jenkins',
      role: 'Managing Partner',
      image: aurelisImages.leadership[0]
    },
    {
      name: 'Robert H. Chen',
      role: 'Senior Director, M&A',
      image: aurelisImages.leadership[1]
    },
    {
      name: 'Victoria Vance',
      role: 'Head of Governance',
      image: aurelisImages.leadership[2]
    }
  ],
  caseStudies: [
    {
      category: 'RETAIL LOGISTICS',
      title: 'Global Supply Chain Alignment',
      desc: 'Redesigning supply chains to bypass global bottlenecks, reducing delivery lead times by 24%.',
      image: aurelisImages.projects[0]
    },
    {
      category: 'FINANCIAL SERVICES',
      title: 'Capital Restructuring for Bancorp',
      desc: 'How we rebuilt the debt framework for a tier-1 retail bank, saving $120M in annualized interest costs.',
      image: aurelisImages.projects[1]
    }
  ],
  testimonials: [
    {
      quote: "Aurelis redefined how we approached our expansion into Asia. Their structured M&A guidance was both rigorous and practical.",
      name: "Eleanor Vance",
      role: "Chief Financial Officer",
      company: "Omnicorp Logistics"
    },
    {
      quote: "Their risk advisory assessment saved us from a costly structural misalignment. I highly recommend their executive consulting.",
      name: "David K. Vance",
      role: "Chairman of the Board",
      company: "Apex Industries"
    }
  ]
};
