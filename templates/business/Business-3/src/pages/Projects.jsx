    ...
    <AnimatedPage>
      <div className="pt-24 pb-20 overflow-hidden">
        ...
        <section className="relative max-w-7xl mx-auto px-6 py-12 text-center flex flex-col items-center gap-6">
          <div className="absolute top-[-30%] left-[10%] w-96 h-96 bg-[#7C3AED]/10 rounded-full blur-[100px] -z-10" />
        ...
        <section className="max-w-7xl mx-auto px-6 mb-12 flex flex-col md:flex-row justify-between items-center gap-6">
              ...
              <button
                ...
                className={`px-4.5 py-2 rounded-full text-xs font-bold transition-all cursor-pointer ${
                  selectedCategory === cat
                    ? 'gradient-bg text-white shadow-md border border-transparent'
                    : 'bg-lightAccent border border-customBorder text-secondaryText hover:text-primaryText hover:border-purple-400'
                }`}
              >
                {cat}
              </button>
          ...
          <div className="relative w-full md:w-80">
            ...
            <input
              ...
              className="w-full bg-lightAccent border border-customBorder rounded-full pl-10 pr-4 py-2 text-sm text-primaryText placeholder-secondaryText focus:outline-none focus:border-primaryAccent focus:ring-1 focus:ring-primaryAccent"
            />
          </div>
        </section>
            ...
            <AnimatePresence mode="popLayout">
              {filteredProjects.map((p) => (
                <motion.div
                  ...
                  key={p.id}
                  className="bg-lightAccent border border-customBorder rounded-3xl overflow-hidden shadow-sm hover:shadow-md transition-shadow group flex flex-col h-full"
                >
                  <div className="relative aspect-video overflow-hidden">
                    ...
                    />
                    <div className="absolute top-4 left-4 bg-[#2E1A47]/90 backdrop-blur-sm px-3.5 py-1 rounded-full text-[10px] font-bold uppercase text-primaryAccent tracking-wider border border-customBorder">
                      {p.category}
                    </div>
                  </div>
                    ...
                    {p.techStack && (
                      <div className="flex flex-wrap gap-1.5 mt-1">
                        {p.techStack.split(',').map((tech) => (
                          <span key={tech} className="text-[10px] font-mono bg-[#1E1033] text-secondaryText px-2 py-0.5 rounded border border-customBorder">
                            {tech.trim()}
                          </span>
                        ))}
                      </div>
                    )}
                    <div className="mt-auto pt-5 border-t border-customBorder">
                      <Link 
                        to={`/projects/${p.id}`} 
