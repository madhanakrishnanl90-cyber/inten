...
import { ADVISORY_SCENARIOS } from '../data/scenarios';

export const AdvisoryPage: React.FC = () => {
  const service = SERVICES.find(s => s.id === 'corporate-advisory') || SERVICES[1];
        ...
        <div className="space-y-6 max-w-4xl">
          ...
          <div className="pt-4 flex flex-wrap gap-4 items-center">
            ...
            <div className="text-xs font-mono text-[#191919]/50">
              Unconflicted Fiduciary • Cross-Border M&A & Valuations
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6 pt-12 border-t border-[#E5E5E5]">
          <div className="space-y-2">
            <span className="text-[10px] uppercase tracking-widest text-[#191919]/50 font-mono block">
              SEQUENCED STRATEGIC PATHWAYS
            </span>
            <h2 className="font-serif text-2xl sm:text-3xl text-[#191919] font-normal">
              How we execute advisory mandates
            </h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {ADVISORY_SCENARIOS.slice(0, 3).map((scen) => (
              <div key={scen.id} className="p-6 bg-[#F4F3F3] rounded-xl space-y-4 flex flex-col justify-between">
                <div className="space-y-2">
                  <span className="text-[10px] font-mono text-[#191919]/50 uppercase">{scen.category}</span>
                  <h3 className="font-serif text-xl text-[#191919] font-normal">{scen.title}</h3>
                  <p className="text-xs text-[#191919]/70 leading-relaxed font-light">{scen.challengeStatement}</p>
                </div>
                <div className="pt-4 border-t border-[#E5E5E5] text-xs font-mono text-[#191919]/60">
                  Target: {scen.keyMetric}
                </div>
              </div>
            ))}
          </div>
