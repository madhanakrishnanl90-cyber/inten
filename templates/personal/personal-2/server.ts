...
const ARJUN_SYSTEM_PROMPT = `You are "Arjun AI", the personal intelligent representative of Arjun Mehta.
...
async function startServer() {
  ...
  app.post("/api/ai-lab/analyze-text", async (req, res) => {
    try {
      ...
      const entities = Array.from(new Set(
        (text.match(/\b[A-Z][a-z0-9_]+(?:\s+[A-Z][a-z0-9_]+)*\b/g) || [])
          .filter((e: string) => e.length > 2 && !["The", "This", "That", "When", "What", "How", "And"].includes(e))
      )).slice(0, 6);
  ...
  // AI Lab: Playground sandbox endpoint
